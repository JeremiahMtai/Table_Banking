...
<div class="container-fluid">
	...
	<form action="" id="loan-application">
				...
				<select name="member_id" id="member_id" class="custom-select browser-default select2">
					<option value=""></option>
						<?php while($row = $member->fetch_assoc()): ?>
							<option value="<?php echo $row['id'] ?>" <?php echo isset($member_id) && $member_id == $row['id'] ? "selected" : '' ?>><?php echo $row['name'] . ' | SaveID:'.$row['save_id'] ?></option>
						<?php endwhile; ?>
				</select>
			</div>
			<div class="col-md-6">
				<label class="control-label">Savings Type</label>
				<?php
				$type = $conn->query("SELECT * FROM save_type order by `type_name` desc ");
				?>
				<select name="save_type_id" id="save_type_id" class="custom-select browser-default select2">
					<option value=""></option>
						<?php while($row = $type->fetch_assoc()): ?>
							<option value="<?php echo $row['id'] ?>" <?php echo isset($save_type_id) && $save_type_id == $row['id'] ? "selected" : '' ?>><?php echo $row['type_name'] ?></option>
						<?php endwhile; ?>
				</select>
			</div>
		...
		<div class="row">
			<div class="col-md-6">
				<label class="control-label">Saving Plan</label>
				<?php
				$plan = $conn->query("SELECT * FROM save_plan order by `week` desc ");
				?>
				<select name="save_plan_id" id="save_plan_id" class="custom-select browser-default select2">
					<option value=""></option>
						<?php while($row = $plan->fetch_assoc()): ?>
							<option value="<?php echo $row['id'] ?>" <?php echo isset($plan_id) && $plan_id == $row['id'] ? "selected" : '' ?> data-months="<?php echo $row['week'] ?>" data-interest_percentage="<?php echo $row['interest_percentage'] ?>" data-penalty_rate="<?php echo $row['penalty_rate'] ?>"><?php echo $row['months'] . ' month/s [ '.$row['interest_percentage'].'%, '.$row['penalty_rate'].'% ]' ?></option>
						<?php endwhile; ?>
				</select>
