...
Class Action {
	...
	function delete_user(){
		...
		$save = $this->db->query("DELETE * FROM users set ".$data." where id = ".$id);
	...
	// start of save_save_type
	function save_save_type(){
		extract($_POST);
		$data = " type_name = '$type_name' ";
		$data .= " , description = '$description' ";
		if(empty($id)){
			$save = $this->db->query("INSERT INTO save_type set ".$data);
		}else{
			$save = $this->db->query("UPDATE save_type set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	function delete_save_type(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM save_type where id = ".$id);
		if($delete)
			return 1;
	}
	// end of save_save_type
